...
- **T-296 — Task 28's premise did not hold, so it became the thing it was pointing at.** The task was written as "a Greek drafts consistency pass on `/el`". **There are no Greek drafts and there is no `/el`** — verified, not assumed: no Greek character sequence exists anywhere in `content/` or `src/`, and no `/el` route exists. That is by design. `content/url-map.md` §5.3 records that **the legacy site is English-only**, so nothing Greek was captured in Phase 0 and writing any would be inventing content about a real property.
  Auditing drafts that do not exist would have produced a clean report about nothing — the exact shape of failure §18 is about. So the deliverable is what the task was for: **`TRANSLATION-BRIEF.md`**, generated by `npm run brief`. **1,123 distinct strings, 7,630 words** across eleven routes, counted from the **rendered** pages — a brief built from `content/` alone would omit every word written in JSX, which on this site is most of the display copy.
  The valuable half is **what must not be translated**, because that is the part a translator cannot know and the part that breaks a booking engine: the operating licence, the reserve-online host, both phone numbers, both addresses, every measurement, and the villa names — all **derived** from the registry and by shape, so a sixth villa is protected on the day it is added.
  **One classification error, found by reading the output.** The first version marked any string containing a protected token as do-not-translate, which swallowed two whole sentences — *"Become one of us and send your CV at creteholidayhome@gmail.com"* and a clause of the terms. A brief that tells a translator to skip a sentence because of one word inside it is worse than no brief. It now distinguishes a string that **is** a token from one that **contains** one, and lists the second kind separately.
  The document also states the four things that must be true before `/el` ships, and what the guard does if you publish early: `tests/locale.spec.ts` turns red rather than serving a locale of 404s to a crawler.
  *Files:* `scripts/translation-brief.mjs`, `TRANSLATION-BRIEF.md`

- **T-297 — Eleven experience pages existed, were indexable, and could not be reached from anywhere on the site.** The link crawl found it on its first run: 24 internal destinations reachable from `/`, against 35 routes that exist.
  The two-tier register is right — fourteen of the twenty-one experiences have under forty words and expanding in place reads better than a page of white space — but **all twenty-one are routed, sitemapped and given a share card**, and only the seven flagship rows carried a link. The other fourteen were reachable only by accident, when another detail page happened to list them as related.
  A page a crawler can index and a visitor cannot reach is the worst of both: thin content in the index, and a dead end for anyone arriving on it from a legacy URL — and legacy URLs point at several of them. The compact panel now carries a quiet "Open the page" link. The register keeps its shape; the page keeps its way in. **Re-crawled: 35 destinations, 21 of 21 experiences, 0 orphans, 0 broken.**
  *Files:* `src/components/sections/DragRegister.tsx`, `src/app/direction-d.css`, `scripts/link-crawl.mjs`

- **T-298 — The Gallery was in the nav and not in the sitemap.** The villas and the experiences were derived from the registry from the start; the eight static routes were typed by hand. So when the Gallery shipped in Task 10 it got a route, a nav entry and a share card, and **never entered the sitemap** — in a file whose own comment says a hand-maintained list is a second source that drifts. It drifted.
  Found by comparing the crawl against the sitemap: 35 reachable, 34 listed. Every static route is now discovered from the filesystem, so a page added next month is in the sitemap the day it is created. Anything left out is named **with its reason** — an exclusion you have to write down is one someone can argue with. `/styleguide` is the only one.
  `tests/sitemap.spec.ts` guards it in both directions: every route on disk is sitemapped or explicitly excluded, and **nothing in the sitemap 404s** — telling a search engine to fetch a page that does not exist is the same defect as an unpublished-locale hreflang (T-280), and a sitemap is the loudest possible way to do it.
  *Files:* `src/app/sitemap.ts`, `tests/sitemap.spec.ts`

- **T-299 — ALL FIFTEEN REDIRECT GAPS CLOSED, INCLUDING THE LOOP.** These were the pre-DNS gate in `LAUNCH.md` §1 and had been open since Phase 0. The parity certificate now reads **Legacy URLs 51 / 51**, and `content/redirect-gaps.json` is empty.
  **None was closed by building a route to satisfy the map.** Every one was repointed at a page that already exists and already carries the content: five `/gallery` sub-pages to their villa or estate page, four `/en/amenities/*` to the estate or the location page, five experience slugs to the inventory's own slug, and `/en/thalasses-rituals` to `/en/weddings`.
  **That last one was a LOOP, not a 404** — the path was both a source and a target in the Phase 0 map, so it pointed at itself. A 404 loses one page; a loop hangs the browser. D1a moved the venue to the Weddings route, so the URL follows the content.
  Every row carries its reason in `content/url-map.md`, which stays the owner's document and the single source the redirect table is derived from. **One owner question survives**, and it is not a gap because the redirect resolves — it is a question about whether it resolves to the right page: `/en/jet-ski-safari-1.html` was titled "Water Sports" and there is no combined water-sports page in the inventory.
  *Files:* `content/url-map.md`, `content/redirect-gaps.json`, `LAUNCH.md`

- **T-300 — The noindex flip, rehearsed rather than described.** `LAUNCH.md` said to replace the blanket disallow "with your live rules" — which is a sentence, not a procedure, and the step it precedes is the one that decides whether a migration keeps its rankings.
...
- **T-304 — The estate served 48 photographs under 3 descriptions, 46 of them identical, and axe passed it every time.** A screen-reader user heard *"The Entire Estate, Thalasses Villas"* forty-six times in a row for the photographic heart of the page. The wedding venue served 31 the same way. Between them, **77 photographs said one sentence.**
  **axe cannot catch this and never could.** It checks that an `alt` attribute exists. Whether it *says* anything depends on what else is on the page, which is not a question a generic engine can ask — and that is exactly how this survived eleven routes of automated accessibility testing on every run for the whole project.
  The cause was a fallback that looked entirely reasonable: `img.caption ?? villaName`. Villa Thoi carries captions on 37 of 40 frames and Eeanthe on 43 of 45, so on those pages it almost never fired and the code read fine. **The estate and Rituals carry zero captions**, so on those two it fired every single time.
  `src/lib/alt.ts` resolves a description from what the inventory already knows, and **never invents a specific**: the frame's own Phase 0 caption; else the curated `subject` written by eye during the photo grading (72 frames have one); else **which house it shows** — the estate's gallery is four albums, one per villa, so an "anonymous" estate frame was never anonymous at all; else an honest positional line that at least distinguishes it and leaves the gap visible.
  **Three refinements, each found by reading the output rather than trusting it.** A generic album title is refused — Rituals' single album is called "Photo gallery", which fired on all 31 frames and produced the original defect with an extra clause bolted on. A description the **inventory itself repeats** (the legacy site captioned four frames "Easy access to our Private beach") keeps its words and gains its position. And the gallery needed a second dedupe at PAGE level, because each villa cluster deduped cleanly while the villas share captions between them.
  **Result: the estate 3 → 48 distinct descriptions for 48 images; weddings 6 → 33 for 33; the gallery 22 → 66.**
  *Files:* `src/lib/alt.ts`, `src/components/sections/TheRun.tsx`, `src/app/en/gallery/page.tsx`, `src/app/en/{the-estate,weddings,villas/[slug]}/page.tsx`

- **T-305 — The guard axe cannot be.** `tests/alt-text.spec.ts`, eleven routes, four rules — and the third is the one that mattered: **no description may repeat more than twice on a page.** Twice is tolerated because the inventory genuinely contains a few duplicate captions; three is a pattern, and a pattern means a fallback is firing.
  It also asserts that an empty `alt` — the correct HTML for "decorative, skip me" — is backed by adjacent text that actually labels the image. **That check was wrong first:** it used `closest()` and stopped at the nearest matching ancestor, and the register's cards wrap their image in a bare `div` with the name one level up, so it reported nineteen perfectly-labelled images as unlabelled. Fixed by climbing until it finds text.
...
- **T-316 — THE GREEK CORPUS. ~18,000 words, eight domains, one voice.** `content/el/` holds the whole content corpus in Greek: all 21 experiences, all seven villa and venue registries, the six amenity tabs with 28 groups, 163 feature names and **57 tooltip descriptions**, every story beat, the homepage litany and acts, location, weddings, careers, contact, gallery, the full site chrome including aria labels, and the terms.
  **Translating existing facts is not inventing them** — the facts are identical, only the language changes. That is the distinction that makes this admissible where writing Greek copy from nothing would not be.
  **`content/el/GLOSSARY.md` is the spine**, written before a word was translated: what must never be translated (the brand, the villa proper nouns, the licence number, contact details, booking URLs, manufacturer names); the voice rules in Greek (υποτονισμός, συγκεκριμένα ουσιαστικά, δεύτερο πρόσωπο πληθυντικός ευγενείας); the `-οντας` register for every clause tail, decided once and listed; and one agreed rendering for every recurring term. Eight translators without a shared glossary produce eight brands.
  **Three clause tails have no natural `-οντας` form** — Retreating, Marrying, Wandering. Each is flagged for the owner rather than forced into a participle that Greek does not actually say.
  **Everything is `copyStatus: "draft"`, and 371 questions are collected for the owner** in `content/el/RECONCILIATION.md` — every term with two defensible renderings, and every place where English is ambiguous and Greek grammar forces a choice of gender, case or number.
  *Files:* `content/el/*` (10 files, 361 KB), `src/lib/i18n.ts`

- **T-317 — The reconciliation caught what one translator never could.** Eight domains in parallel, then a pass that did not re-translate anything and looked only for disagreement. **12 corrections in 3 files.** The brand's grammatical gender was the real one: `experiences.json` alone treated *Thalasses Villas* as neuter plural («Στα»), while `villas.json` and `pages.json` had both settled on feminine plural agreeing with «οι βίλες» — the rest of the corpus had already decided, and one file had not. Also: the same body of water as «κρητικό πέλαγος» in one file and «Κρητικό Πέλαγος» in another, and one draft badge written three different ways for a single English string.
  *Files:* `content/el/RECONCILIATION.md`

- **T-318 — AND THEN I CHECKED IT MYSELF, AND THE REPORT WAS WRONG.** `RECONCILIATION.md` reported the numerals clean. It was written by the process that produced the corpus, and a report from the thing being checked is a claim (§18). `scripts/verify-greek.mjs` re-derives the answer from both sides — and found **three figure mismatches the reconciliation had passed**.
  The cause was structural, not linguistic. **Descriptions were keyed by featureId, and the same featureId carries different English in different villa files.** The estate says *"4 Fully equipped kitchens with branded electric devices"* where a villa says *"Fully equipped kitchen"*; featureId 366 has **five** separate wordings of the pool sentence. A featureId map holds one Greek — so the estate page would have rendered the single-villa text and told a Greek reader there was **one kitchen**.
  **57 distinct English texts, 42 Greek entries. Fifteen collisions, every one silent.** Re-keyed by the English string, which is 1:1 by construction, and the 22 untranslated variants were translated by hand. **Now 57 of 57**, and the verifier reads clean on all six checks.
  **My own verifier was wrong twice first**, and both were the same mistake in different clothes: it read array indices (`highlights[0]`) as English keys and reported 128 false positives, then read featureId references (`d498`) the same way and reported 25 more. 131 findings, of which 3 were real — a check that cries wolf 128 times is a check nobody reads to the end of.
  *Files:* `scripts/verify-greek.mjs`, `content/el/facilities.json`

- **T-319 — The corpus is guarded, and it is not published.** `tests/greek-corpus.spec.ts` runs the same six checks inside the suite so they cannot be skipped: every file parses and declares itself a draft **and raises notes** (a large translation with nothing for the owner to rule on means nobody looked); the terms carry `[legal draft — owner review mandatory]`; **no figure differs from its English source**, across 148 paired comparisons; proper nouns survive; Greek capitals carry no accents; the banned words are absent; and every experience, villa and feature id is covered.
...
- **T-325 — Visual baselines: 22 approved viewports, and the photography is masked out of them.** Everything else in this suite asserts a fact — a figure matches, a link resolves, a contrast clears. None of it can see a collapsed section, a heading landing on a rule, or a grid losing a column. Worth having only now: a baseline taken over a design still being argued about is a machine for false failures, and Direction D has been stable for two tranches.
  **Masking the images is the decision that matters.** Unmasked, the 22 baselines came to **13.4 MB**, and losslessly recompressing them saved 3% — a PNG of a photograph is already about as small as it gets. Committing 13 MB of JPEG noise into a repository that just had 167 MB reclaimed from it would be a poor trade, and it is also **the wrong thing to store**: the photography is already guarded (`images.spec` asserts every image serves; `alt-text.spec` asserts every one is described), while nothing guarded layout. Masked, the baseline is about type, spacing and structure and nothing else — and each image's BOX is still compared, so one that changes aspect or loses its container still fails. **13.4 MB → 4.0 MB.**
  Viewport rather than `fullPage` (the homepage is 20,000 px and thirty full-page shots killed this runner once), reduced motion emulated (a real rendering path the site supports, not a test mode), and `deviceScaleFactor` 1 (a retina baseline is four times the bytes for no extra signal).
  **Weight delta: 304.9 MB → 309.6 MB tracked. Suite: +22 tests, ~50s.**
  *Files:* `tests/visual.spec.ts`, `tests/visual.spec.ts-snapshots/*` (22)

- **T-326 — The baseline hung the homepage for 30 seconds, and `.catch()` could not help.** Every other route passed in three; the homepage timed out. Twelve images are left **permanently incomplete** by a scroll-through — they are `loading="lazy"`, their requests are abandoned when they leave the viewport, and **`img.decode()` on an abandoned request never settles**. It does not reject either, so the `.catch(() => {})` around it was decorative. A hang, not a slow test, and no amount of raising the timeout fixes a promise that will not resolve. Bounded with a race; `toHaveScreenshot` already waits for two identical frames, so decoding only makes it settle sooner.
  Two smaller ones in the same sitting: a `snapshotPathTemplate` override I added — with a comment claiming it controlled `deviceScaleFactor`, which it does not — wrote every baseline to **`C:isual.spec.ts-snapshots\`**, the drive root. Playwright's default already places them beside the test; the override was wrong and unnecessary and is gone.
  *Files:* `tests/visual.spec.ts`, `playwright.config.ts`

- **T-327 — THE FIRST TWO FALSIFICATIONS PASSED, AND THAT WAS THE USEFUL PART.** Changing `.c1 { line-height }` from 1.05 to 1.14, and then to 1.40, left the homepage baseline green — so I measured it directly: **zero differing pixels of 1,296,000.** The hero clause's line-height is overridden elsewhere in the cascade, so I had been falsifying a rule that does not apply to the thing I was looking at.
  My first instinct was that the tolerance was too loose. It was not — the tolerance allows 2,592 pixels and the real diff was **zero**. Measuring before adjusting is what stopped me tightening a threshold to "fix" a test that was reporting the truth.
  Falsified properly by moving `--section-y`, the rhythm token every beat uses, by 1rem: **12 of the 22 baselines fail.** The ten that pass are viewports whose first screen is a full-bleed hero with no section boundary in it — honest, and the limit of a viewport baseline, stated rather than hidden.
