...
The cost of the experiment, recorded for the owner's yes or no.

> **Merged into main under D-021, behind the provenance gate.** This record was
> written on `feat/estate-3d` before the owner's approval. Its figures are the
> branch build's, measured before `content/estate-plan.json` and the gate
> existed.
>
> On main, the public page does not load three.js at all while the plan is
> unverified (`tests/estate-3d.spec.ts`). The diagram runs only on the local
> review build (`npm run build:estate3d`).
>
> The owner's ruling also asks for INP with the map active. That measurement,
> and any re-measure on main, is recorded separately and does not revise the
> figures below.
...
- **When the chunk is fetched.** All three conditions must hold:
  ...
  **On main, a condition comes before all three.** The provenance gate
  (`src/lib/estate-plan-gate.ts`, D-021, D-022) must be open, which means every
  drawn element is owner-verified. It is decided at build time. While it is
  closed the page passes no render plan, and none of the three checks runs.
  Since the gate, the WebGL check asks for WebGL2 only.
