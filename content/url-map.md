...
| `/en/property/200/albums.html` | Villa Thoi — album | `/en/villas/villa-thoi/gallery` | 301 | `CANONICAL: albums.html` (relative, non-unique — all five album pages declare the identical canonical value). |
| `/en/property/201/albums.html` | Villa Persi — album | `/en/villas/villa-persi/gallery` | 301 | Same non-unique `albums.html` canonical. |
| `/en/property/202/albums.html` | Villa Eeanthe — album | `/en/villas/villa-eeanthe/gallery` | 301 | Same non-unique `albums.html` canonical. |
| `/en/property/203/albums.html` | Villa Melia — album | `/en/villas/villa-melia/gallery` | 301 | Same non-unique `albums.html` canonical. |
| `/en/property/2142/albums.html` | Thalasses Villas — album | `/en/the-estate/gallery` | 301 | Same non-unique `albums.html` canonical. |
...
| `/en/thalasses-rituals-1.html` | Thalasses Rituals (wedding venue) | `/en/thalasses-rituals` | 301 | `villas/rituals.json`, internal id 7798, `pageType: landing`. Not bookable. Keeps its own top-level route — it is a venue, not a villa. |
| `/en/article/1899-1.html` | Sitting outdoor area (amenity article) | `/en/amenities/sitting-outdoor-area` | 301 | The **only** amenity with a real crawlable page; the other five amenity cards open Angular modals. `sourceTaxonomy: "Amenities"` (every experience page reads `Experiences`). 58-image album with zero captions. |
...
| `/en/biological-garden-1.html` | **Organic Farm** | `/en/experiences/organic-farm` | 301 | **SLUG/NAME MISMATCH — legacy URL must keep working.** Page `<title>`, `og:title` and homepage card all read "Organic Farm"; the URL still says `biological-garden`. New slug follows the displayed name. |
| `/en/boat-trip-1.html` | **Private Boat Trip** | `/en/experiences/private-boat-trip` | 301 | **SLUG/NAME MISMATCH — legacy URL must keep working.** Title is "Private Boat Trip", URL is `boat-trip`. |
...
| `/en/chef-in-villa-1.html` | **Private Chef** | `/en/experiences/private-chef` | 301 | **SLUG/NAME MISMATCH — legacy URL must keep working.** Title is "Private Chef", URL is `chef-in-villa`. Distinct from the "Cook (professional chef) in the villa" *service* card (see §4). |
| `/en/dream-weadding-on-the-beach-1.html` | Dream Wedding on the Beach | `/en/experiences/dream-wedding-on-the-beach` | 301 | **SOURCE TYPO IN A LIVE, INDEXABLE URL — "weadding".** The legacy path is preserved verbatim in this table and *must* be kept as a redirect source forever; the new slug is corrected to `dream-wedding`. Never rewrite the legacy string. |
...
| `/en/jet-ski-safari-1.html` | **Water Sports** | `/en/experiences/water-sports` | 301 | **SLUG/NAME MISMATCH — legacy URL must keep working.** Title is "Water Sports", URL is `jet-ski-safari`. The widest gap between slug and name on the site. |
...
| `/en/property/<id>/albums` | Album pages, extensionless | `/en/villas/<slug>/gallery` (2142 → `/en/the-estate/gallery`) | 301 (pattern) | Same extensionless/`.html` duality. Ids 200, 201, 202, 203, 2142. |
...
| `/en/thalasses-rituals` | Thalasses Rituals, extensionless | `/en/thalasses-rituals` | 301 | `villas/rituals.json → legacyUrls: ["/en/thalasses-rituals"]`. Legacy and new path are identical strings — still declare the redirect so the `-1.html` form and this form both land on one canonical URL. |
...
| `/en/article/1899` | Sitting outdoor area, extensionless | `/en/amenities/sitting-outdoor-area` | 301 | `article-1899.json → legacyUrl: "https://thalasses.com/en/article/1899"`. |
| `/en/page/modal/article/1896` | Private Beach (modal) | `/en/amenities/private-beach` | 301 | **Returns 404 on the live host** — the site is a static export and the modal HTML is baked into the page bundle, injected by Angular. No inbound equity, but the content is real (`modals.json`) and gets a URL for the first time. Add the redirect defensively in case the CMS pattern was ever indexed. |
...
| `/en/page/modal/article/3795` | Doctor (modal) | `/en/amenities/doctor` | 301 | 404 today. |
| `/en/page/modal/article/3796` | Babysitter (modal) | `/en/amenities/babysitter` | 301 | 404 today. |
