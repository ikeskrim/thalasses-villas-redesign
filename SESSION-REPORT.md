...
**Holding, still, for the owner's Google Drive link and the site plan.** Everything in the rulings that did not depend on either was built and pushed, step by step, a commit per step and each pushed on its own — **except the two measurements the rulings themselves asked for**: the INP re-measurement of the tap, and the Framer lab A/B. Neither depends on the Drive link or the plan, and both are named as owed below.

Every claim below rests on a committed record or on a check named beside it. **Production checks exist for the encoded-redirect commits and the Framer commits** (`qa/security/ENCODING-tranche14.md` §6, `qa/perf/FRAMER-tranche14.md` §3); the Villa Pueblo copy change was checked against a local build only. **The 3D code is now checked on production too:** Vercel cancelled two queued builds overnight and deployed f4cb430, and `check-estate-gate` against that deployment PASSES — gate closed on both conditions, the page serving its nine 2D markers and no render plan in either the HTML or the flight data (2026-09-19T11:16Z).
...
### The instrument exists; the measurement is not taken

**The harness that writes the record landed in 267224e (D-036).** Its rules live in a pure module whose self-check runs the gate's *own* reader against the record it is about to write, so "the harness refuses to write a record the gate would reject" holds by construction. A shape failure is not written at all; a substance failure — too few trials, or a trial over the bar — **is** written, because a figure over 200 ms is evidence and is never withheld. It measures 23 cells per profile per arm: a no-interaction control checked three ways, the first interaction's own latency, the five fixed scenarios, and eight window families at two deliberately chosen offsets each. A smoke run exercised every one of them and exited non-zero naming each shortfall, which is what it should do.

Two things it settled on the way. It had to tap a list number before waiting for the diagram — since the runtime landed, nothing loads without a first interaction, so without that every review-build trial would have been invalid. And it escalated one interpretation rather than choosing quietly: writing `fetched: false` on every no-entry trial, as the gate note said literally, would be false on a review-build trial where the chunk was fetched, and the gate never checks that field outside control rows. It is now recorded truthfully everywhere (D-036).

**What remains is the measurement itself, and an adversarial review of the instrument before it.** Hours of exclusive machine time spent on a flawed instrument is a day lost, and the failure that matters most here — a record the gate accepts that does not mean what the gate thinks — is exactly the kind a smoke run cannot catch.

The leading risk is unchanged, and it is already visible in the phase timings: **the three.js chunk's own evaluation took 83–104 ms of the 200 ms budget** in the three phone-profile runs the record carries — one V8 task that no amount of yielding can enter, and smoke figures taken by the runtime stage in its own worktree on a shared machine (an earlier build of the same stage measured 105–168 ms in a run that is not in the record, so the true spread is wider than the three runs suggest). If the measurement fails, it will most likely fail there, and the honest levers are a smaller three.js subset or a different bar, not a cleverer wait. Warming the chunk separately was considered and refused with reasons: application code cannot name the file a dynamic `import()` will fetch, and on localhost the download is a few milliseconds of that figure anyway.

Until the record exists the map stays closed on **both** conditions, and `check-estate-gate` confirms — on every build **and on the deployment carrying this tranche's code** — that what is served is what the gate decides: the 2D map, with no render plan in the HTML or the flight data.
