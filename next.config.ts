...
import encoded from "./src/generated/encoded-redirects.json";
 ...
 * against the percent-decoded path, so `/en/%63ontact.html` matches both, and
 * the proxy sends it this same static policy (src/proxy.ts). (`/en/%63ontact`
 * itself no longer gets that far: the redirects below answer it first.)
 */
...
const nextConfig: NextConfig = {
   ...
   *
   * THEN THE PERCENT-ENCODED DUPLICATES (DECISIONS.md D-028): one 301 per
   * canonical page, generated by scripts/build-encoded-redirects.mjs into a file
   * of their own, so the legacy harness above and the parity counts never see
   * them. Each source accepts every character of the page's path or its `%XX`
   * escape and requires a `%`; each destination is the literal path.
   *
   * Why here and not in src/proxy.ts: Next matches config redirects before the
   * proxy and the filesystem, against the raw path (measured under
   * `next start`; on Vercel the raw-path, case-insensitive matching was probed
   * with the legacy rules), and they put no function in front of any URL. A
   * proxy matcher broad enough to see these spellings would run a metered
   * function ahead of the CDN on every page view, which is what D-012 and D-016
   * protect the prerendered pages from. The generator refuses a list in which
   * any destination would match any rule, so neither list can loop through the
   * other.
   *
   * Two consequences, both deliberate: Next matches these sources
   * case-insensitively, so `/EN/%74he-estate` is redirected too (always to the
   * fixed lower-case page); and `next start` sends none of headers() below on a
   * config redirect, while Vercel does. Next keeps config redirects off
   * `/_next/**`, so the `_next/data` spelling of the contact page is still
   * answered by the proxy (see there).
   */
  async redirects() {
    return [...generated.redirects, ...encoded.redirects];
  },
  async headers() {
    return [
