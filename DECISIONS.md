...
### D-035 · The 3D diagram loads after a first interaction, at idle, in yielding tasks (carrying out D-028)

D-028 refused the tap cost of the first implementation and said what to do: "defer the three.js load until idle and after first interaction, split the first-frame work into yielding tasks, re-measure; the gate stays closed until the tap stays under 200 ms." The runtime is 25f4e51; the evidence is `qa/perf/ESTATE3D-runtime-tranche14.md`; **the measurement of record is not taken yet**, so the INP condition still reports "no record" and the public map stays closed on both conditions.

**Build defaults — mine, recorded with the reason, and the owner's to overturn.**

1. **What counts as the first interaction:** `pointerup` and `keydown` on `window`, passive and capturing, removed after the first event, page-wide. NOT `pointerdown`, `touchstart`, `scroll`, `wheel` or `touchmove` — a touch that becomes a scroll ends in `pointercancel` and never in `pointerup`, which is also what Event Timing counts.
2. **The quiet period** is 1000 ms, restarted by any `pointerdown`, `keydown`, `wheel`, `touchmove` or `scroll`, and re-armed from a lazily read timestamp so a scroll does not reset a timer on every event.
3. **Idle** is `requestIdleCallback` with a 2000 ms timeout, and a 200 ms timer where it does not exist (Safari). Both constants are exported and both specs read them out of the source, so a change cannot leave a spec waiting zero.
4. **The WebGL2 probe runs in the idle step, before any fetch:** a device without WebGL2 downloads nothing. Data Saver is re-checked there as well as at registration.
5. **`renderer.debug.checkShaderErrors` is on in development and off in every production build**, the local review build included — because the build the harness measures has to be the build the public would get. It hides nothing: the pipeline reads each program's `LINK_STATUS` itself and falls back to the 2D map on failure.
6. **The swap is held** while a 2D card is open, while focus is inside the 2D frame, and while a pointer is down on it; it is retried at the next idle moment after the pointer or focus leaves.
7. **The swap writes the DOM directly and lets React follow** (`data-state`, the diagram's `inert`/`aria-hidden`), with `setPhase` in a transition after it. Safe because React diffs an attribute against its own last rendered props.
8. **`src/lib/schedule.ts` is the map's own scheduler and `HotelMotion` keeps its copies.** Sharing would put the homepage's motion inside the 3D map's INP fingerprint, so a change to one would close the gate on the other. `yieldToMain` uses a `MessageChannel` where `scheduler.yield` is absent, because nested `setTimeout(0)` is clamped to 4 ms after five levels — which a chain of awaited yields reaches at once.
9. **One layout at a time, newest wins on the next landing.** A resize that lands mid-layout does not abandon the layout in flight; that layout completes for the size it searched and the newer size is laid out immediately afterwards. For one chain of tasks the drawing is CSS-stretched over the difference. The alternative — abandon and restart — holds the older drawing on screen for longer and can starve under a continuous resize.
10. **The build's own layout is the only one that marks.** A resize's layout emits no `estate3d:layout`/`estate3d:swap`, so the harness sees exactly one of each per page view.

**What the review caught, and it would have shipped otherwise.** The first implementation had one function doing the whole layout — measure, camera fit, the projection of every anchor and hull, the entire label search, the render and the writes — **in a single un-yielded task**, and the build took that path whenever the frame changed size during the build. A phone does that whenever Chrome collapses its URL bar mid-scroll, and the reader has the whole build to do it in — about 0.3 s of layout inside a ~0.5 s sequence, after the one-second quiet wait; on the phone profile that one task is about 150 ms against a 200 ms bar, and it is the task the gate's record would have claimed to measure. There is now no path that lays out in one task: one read pass, the projection on a cloned camera so a superseded layout leaves nothing behind, a task per search step, then one write-only task — and the `ResizeObserver` is registered before the layout phase rather than after the swap. A test counts the yielded tasks on both paths and times a resize into the middle of the search.

**Two consequences the owner should see, because they are the ruling's and not defects.**

- **A phone reader who only scrolls never sees the diagram.** Scrolling, the wheel and a touch swipe are deliberately not interactions.
- **A screen-reader reader in browse mode may have the 2D markers replaced under them.** The hold reads DOM focus, an open card and a pointer; a virtual cursor moves none of those. Desktop only (the 2D markers are hidden below 768 px), and the numbered list beneath the map carries every place either way.

**What this does not do.** It does not measure the tap: that is the harness (D-033) and a machine doing nothing else. The leading risk is already visible and recorded — the chunk's own evaluation took 83–104 ms of the 200 ms bar in three phone-profile runs, and 105–168 ms in an earlier build of the same stage, in one V8 task no yield can enter — so if the measurement fails it will most likely fail there, and the levers are a smaller three.js or a different bar, which is the owner's call.

### D-036 · The instrument that writes the gate's INP record (carrying out D-033)

D-033 made the gate's second condition mechanical: it reads a committed record and recomputes the verdict from its rows. This entry is the instrument that writes that record. **It does not take the measurement** — that needs hours on a machine doing nothing else, and it is still owed.

- **The rules live in a pure module and the reader is not copied.** `scripts/estate-inp-record.mjs` turns measured rows into the record, and its self-check runs `decideInpCondition` — **the gate's own function**, the one the build and `check-estate-gate` use — against the record it is about to write. So "the harness refuses to write a record the gate would reject" is true by construction and cannot drift. Everything in the module is pure (rows in, record out; no browser, no clock, no filesystem), so every rule is driven by a unit spec in milliseconds instead of after an hour of measurement.
- **The self-check has two verdicts, and the difference is deliberate.**
  - **Shape** — is the record READABLE as the schema (cells complete, ids spelled as the gate spells them, both profiles, calibration passed, this tree's fingerprint)? A shape failure is the harness's own bug, so **the record is not written**: a file the reader cannot parse is not evidence.
  - **Substance** — are there enough valid trials, and is every one under the bar? That is a measurement, so **the record IS written** and the harness exits non-zero naming every reason. **A figure over the 200 ms bar is evidence and is never withheld.**
- **One spelling of the `estate3d:*` mark names**, in `src/components/sections/estate-map-3d-marks.ts`, imported by both components, the preview spec and the harness. It sits under the mount path on purpose, so the fingerprint hashes it: the mark names decide what the record measured, and a record taken against one set of phase names does not describe a tree with another. Four checks keep it honest, including one that forbids any other file under `src/components/sections` from spelling a mark name inline.
- **The trigger tap, without which none of this could be measured.** Since D-035 nothing is fetched until the reader's first completed interaction, so the harness now taps a list number (not a link, not inside the frame) before waiting for the diagram. Before this, every review-build trial would have come back invalid after a 60 s wait for a diagram that could never mount.
- **How a mark is learned while the main thread is blocked.** Each phase mark is written at the START of the task that does that phase's work, so a `PerformanceObserver` callback for it cannot run until that task ENDS — too late to fire an input "20 ms after the renderer mark". On window trials only, and on both builds alike, the harness wraps `performance.mark` so it also `console.log`s the name **synchronously, in the same task**; the DevTools protocol carries that out of a renderer that is still busy. `request` and `arrival` need no wrapper — they are network events seen in the browser process, which is exactly why those two families are not marks: the point of them is to sample the window in which the page is about to be blocked, and a page about to be blocked cannot be relied on to report anything.
- **Nothing is evaluated in the page between an anchor and the input,** because `page.evaluate` needs the main thread and a busy main thread is what these cells exist to catch. Every row carries the offset it ACTUALLY achieved, computed in the page's own clock.
- **The offsets, chosen and not defaulted** (two per family, which is what D-033's arithmetic assumes): `arrival` at 0 and 50 ms straddles the chunk's evaluation deliberately — 0 waits the whole of it, 50 lands inside it; `request` at 0 and 100 ms sits inside the download on both profiles (changed from 250 after a pin run measured the desktop bytes arriving at 175 ms, which would have duplicated `arrival`); `trigger` at 0 and 500 ms, the second a negative control mid quiet-second; `layout` at 0 and 60 ms, the read pass and mid-search. The control waits 12 s, and the harness refuses anything under D-033's 10 s floor.
- **MY RULING on the one interpretation the stage escalated.** The gate-fix note said a trial with no Event Timing entry should be written `under16: true, inpMs: null, fetched: false`. Taken literally for every no-entry trial, `fetched: false` would be **false on a review-build window trial, where the chunk demonstrably was fetched** — and the gate reads `fetched` only on control rows, so the falsehood would go unchecked. The instruction describes the CONTROL trial, for which all three values are exactly right. **`fetched` is recorded truthfully on every row**, and a control that cannot say whether three.js was fetched is recorded `invalid`, because such a control is not evidence. The record is evidence before it is a gate input.
- **A consequence worth knowing before the measurement is taken:** the fingerprint hashes file bytes, so **a comment-only edit to a mount-path source invalidates the record**. One stale spec-file reference fixed in a header moved the tree's fingerprint from `aaafddb4…` to `458fe34c…`. Land documentation and comment changes to those files BEFORE measuring, not after.
- **What is owed before the measurement:** an adversarial review of this instrument. Hours of exclusive machine time spent on a flawed instrument is a day lost, and the failure that matters most — a record the gate accepts that does not mean what the gate thinks — is exactly the kind a review finds and a smoke run does not.

### D-037 · The build yields through a posted message, never `scheduler.yield()` (carrying out D-028)

D-028 said: "split the first-frame work into yielding tasks … the gate stays closed until the tap stays under 200 ms." **The work was split and the tap still waited 241 ms.** `qa/perf/YIELD-tranche14.md` is the measurement; this entry is the change.

- **What was wrong.** A continuation resumed by `scheduler.yield()` keeps the caller's priority, and on this engine that outranks a pending input. So the diagram's build — twelve to thirteen separately yielded steps, pinned by a test — monopolised the main thread exactly as one long task would: a tap at the start of the label search waited **241 ms** of input delay, and the delay fell one-for-one with how late the tap was fired (241, 225, 180, 165 ms at 0, 25, 50, 75 ms into a phase only 82 ms long). The tap was waiting for the END OF THE CHAIN, not for the task it landed in.
- **The change.** `yieldToMain` always posts a `MessageChannel` message. The same tap measures **20 ms**, and on the landed tree the layout cells are 32 and 24 ms against 248 and 232. **It costs nothing measurable:** the layout phase is 82 ms with `scheduler.yield()` and 80 ms without it.
- **`setTimeout(0)` is not the alternative** — nested timers are clamped to 4 ms after five levels, which a chain of awaited yields reaches at once. A posted message is an ordinary task, unclamped, and outranked by input, which is the point.
- **Splitting is necessary and not sufficient,** and the records now say so. A test counts the split (it must, or a regression to one task goes unnoticed); only the INP record says the tap is actually served.
- **Three comments in this repository asserted the opposite** — in `schedule.ts` ("but after input"), in `EstateMap3D.tsx`, and in the preview spec ("input is dispatched ahead of the continuations, which is the whole point of yielding"). The belief is what the API is for; it was never measured here until now. All three are corrected.
- **Four tests would have gone quiet.** They slowed the build, and the resize test counted its split, by wrapping `scheduler.yield()` — which the page no longer calls. They now instrument `MessageChannel` per channel and take the busiest, because React's scheduler uses one too.
- **The tightest cell has moved to the swap.** On the landed tree a tap at the swap mark measures 160 ms against the 200 ms bar, where the layout now measures 32. Something after that mark holds the thread for ~151 ms — more than the write-only task's 37–44 ms — plausibly the first composite of a WebGL canvas this rig draws in software. The measurement of record reports the WORST of twenty trials per cell, so that is the cell most likely to decide the gate, and it is named in advance rather than discovered afterwards.
- **Not established, and recorded rather than assumed:** whether a real Android device and a genuine touch behave as this lab does, and whether Chromium's priority here is intended. Settling the last one needs a minimal reproduction away from this codebase, which would be worth filing upstream.

### D-038 · A swap-only failure is measured again on real devices before the gate is ruled (the owner)

**The ruling, as given:** "If the measurement of record fails ONLY on the swap cell, treat that cell as suspect: the ~151 ms is consistent with software-rendered WebGL on the lab machine. Before ruling the gate, measure that one cell on a real Android and iPhone against the deployed preview (Event Timing, same protocol) and record both; a gate that fails on lab hardware but passes on real devices is opened on the device numbers, with the lab figure kept in the record as the caveat."

**What it settles.** The lab is not the last word on one specific cell. This rig draws WebGL through SwiftShader (software), which makes a first composite far more expensive than a phone GPU's, and `window:swap` is precisely the cell that sits on that composite — D-037 measured 160 ms there against 32 ms at the layout, and named it as the cell most likely to decide the gate. If it is the ONLY failure, the lab figure is evidence about this machine rather than about the map, and **the device numbers decide**, with the lab figure kept beside them.

**The condition is narrow, and it is meant to be.** It applies when the swap cell alone fails. If any other cell fails as well, the lab result stands as it is and the gate stays closed — the ruling gives no licence to explain away a broader failure as "software rendering".

**Two obstacles this session has identified, before any device is touched.**

1. **The deployed preview cannot currently show the diagram at all.** Two guards see to that, both of them deliberate and both owner-approved: `scripts/estate3d-preview.mjs` refuses to run when `VERCEL` is set, and the gate itself refuses `ESTATE_3D_PREVIEW` on Vercel (D-021, D-022), so on the deployment the page is always the 2D map. D-028 also said in terms: "no public preview of an unverified map". A phone pointed at the deployment therefore measures the 2D page, which is not the cell in question.
   - **The way that keeps every ruling intact:** serve the local review build to a phone on the same Wi-Fi — `next start` already reports its LAN address, no deployment, nothing public, no guard touched. The device is real, the build is the review build, the protocol is the harness's.
   - Deploying an unverified preview instead would need D-022 and D-028 reopened, and this session will not assume that.
2. **"Event Timing, same protocol" is probably not available on the iPhone.** The Event Timing API is a Chromium feature; to the best of this session's knowledge WebKit does not implement `PerformanceEventTiming`, and every iOS browser is WebKit. **This will be checked on the device rather than asserted.** If it is absent, the iPhone figure cannot be the same measurement, and the honest options are a documented stand-in (the interval from the touch to the next paint, measured in the page) recorded as a DIFFERENT instrument, or no iPhone figure at all — not a number that looks like the others and is not.

**Not done yet.** The measurement of record has not run; the condition may never arise. This entry is recorded now because the ruling was given now.
